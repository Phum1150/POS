...
const price = ref(Number(props.item.price))
...
const save = async () => {
    error.value = ''
    try {
        await updateProduct(props.item.id, { name: name.value, price: price.value, type: type.value })
        emit('save')
    } catch (err) {
...
<template>
    <BaseModal width="w-[30%]">
        <p class="text-xl font-semibold border-b border-neutral-800 pb-3 mb-4">Edit Product</p>
